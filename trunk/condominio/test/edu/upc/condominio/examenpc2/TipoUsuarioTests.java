...
public class TipoUsuarioTests {
	...
	public void Inicio() throws Exception {
		...
		registroresidente.registrarResidente("12345680", "Cesar Gustavo", "Aguilar Cardenas",32, "cesargustavo2@hotmail.com", "12389");
		...
		regUsuario.registrarUsuario("u002", "Usuario Prueba 2", 
				regTipoUsuario.getTipoUsuario(2), regVivienda.getVivienda("viv0002"), registroresidente.getResidente("12345679"));
		regUsuario.registrarUsuario("u003", "Usuario Prueba 3", 
				regTipoUsuario.getTipoUsuario(3), regVivienda.getVivienda("viv0003"), registroresidente.getResidente("12345677"));
		regUsuario.registrarUsuario("u004", "Usuario Prueba 4", 
				regTipoUsuario.getTipoUsuario(4), regVivienda.getVivienda("viv0004"), registroresidente.getResidente("12345680"));
	...
	public void testTipoUsuarioContacto() {
		...
		try{
		regUsuario.registrarUsuario("u005", 
				"Usuario Prueba 5", 
	...
	public void testTipoUsuarioContactoPrincipal() {
		...
		try{
			regUsuario.registrarUsuario("u006", 
					"Usuario Prueba 6", 
					regTipoUsuario.getTipoUsuario(1), 
					regVivienda.getVivienda("viv0003"), usuario );
			fail();
		}catch(Exception ex){
				assertEquals(ex.getMessage(), "No puede crear usuarios de otras viviendas");
		}
		assertEquals(5, regUsuario.getUsuarios().size());
		...
		try{
			...
			fail();
				...
				assertEquals(ex.getMessage(), "Solo puede crear usuarios de Tipo Contactos");
		}
		assertEquals(5, regUsuario.getUsuarios().size());
		...
		regUsuario.modificarUsuario(regUsuario.getUsuarios("u005"), usuarioModificado, usuario);
		assertEquals("Prueba A Modificar Nombre", regUsuario.getNombreUsuario("u005"));
	...
	public void testTipoUsuarioAdministrador() {
		...
		try{
			...
			fail();
				...
				assertEquals(ex.getMessage(), "El usuario ya se encuentra registrado");
			}
			System.out.println(regUsuario.getUsuarios().size());
			assertEquals(5, regUsuario.getUsuarios().size());
	}
	...
	public void testPublicacionPostMensajesDeUsuarios() {
		...
		usuario = regUsuario.getUsuarios("u001");
		regPost.registraPost("P001", "Post Prueba 1",regSecciones.getSeccion("S01") , usuario);
		assertEquals(1,regPost.getPost().size());
		
		//registrando Post / usuario de tipo contacto principal - Seccion de Tipo Foro
		usuario = regUsuario.getUsuarios("u002");
		regPost.registraPost("P002", "Post Prueba 2",regSecciones.getSeccion("S02") , usuario);
		assertEquals(2,regPost.getPost().size());
		
		//registrando Post / usuario de tipo contacto administrador - Seccion de Tipo Comite
		usuario = new Usuario();
		usuario = regUsuario.getUsuarios("u004");
		regPost.registraPost("P003", "Post Prueba 3",regSecciones.getSeccion("S03") , usuario);
		assertEquals(3,regPost.getPost().size());
		
		//registrando Post / usuario de tipo comite intentando crear Post de tipo Noticia
		usuario = regUsuario.getUsuarios("u003");
		try{
			regPost.registraPost("P004", "Post Prueba 4",regSecciones.getSeccion("S01") , usuario);
			fail();
		}catch(Exception ex){
		...
		//Envio de Mensajes
		//Escribe Mensaje / usuario de tipo contacto principal - Seccion de Tipo Foro
		regMensaje = new RegistrarMensaje();
		usuario = regUsuario.getUsuarios("u001");
		regMensaje.registrarMensaje("M001", regPost.getPost("P001"), "Hola", usuario);
		assertEquals(1,regMensaje.getMensajes().size());
		
		//Escribe Mensaje/ usuario de tipo contacto principal - Seccion de Tipo Foro
		usuario = regUsuario.getUsuarios("u002");
		regMensaje.registrarMensaje("M002", regPost.getPost("P002"), "Hola", usuario);
		assertEquals(2,regMensaje.getMensajes().size());
		
		//registrando Post / usuario de tipo contacto administrador - Seccion de Tipo Comite
		usuario = new Usuario();
		usuario = regUsuario.getUsuarios("u004");
		regMensaje.registrarMensaje("M003", regPost.getPost("P003"), "Hola", usuario);
		assertEquals(3,regMensaje.getMensajes().size());
		
		//Escribe Mensaje / usuario de tipo comite intenta visualizar post de tipo Foro
		usuario = regUsuario.getUsuarios("u003");
		try{
			regMensaje.registrarMensaje("M004", regPost.getPost("P002", usuario), "Foro", usuario);
			fail();
		}catch(Exception ex){
			assertEquals(ex.getMessage(), "Solo Puede Visualizar el Post de Comite");
		}
		assertEquals(3,regMensaje.getMensajes().size());
		
		//Escribe Mensaje / usuario de tipo comite intentando escribir en post de comite 
		usuario = regUsuario.getUsuarios("u003");
		try{
			regMensaje.registrarMensaje("M004", regPost.getPost("P003", usuario), "Comite", usuario);
			fail();
		}catch(Exception ex){
			assertEquals(ex.getMessage(), "No Puede Escribir Mensajes solo Visualizar");
		}
		assertEquals(3,regMensaje.getMensajes().size());
